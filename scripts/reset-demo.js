...
// Sample products (same as in seed.js)
...
// Sample coupons (same as in seed.js)
const sampleCoupons = [
  {
    code: 'WELCOME10',
...
/**
 * Reset demo: Clear all transactional data (Orders, Conversations, Carts)
 * and re-seed Products and Coupons to a clean state.
 * Useful for resetting between demo runs.
 */
const resetDemo = async () => {
  try {
    const mongoUri = process.env.MONGO_URI || 'mongodb://localhost:27017/ai_shopping_agent';
    console.log(`[Reset-Demo] Connecting to MongoDB: ${mongoUri}`);
    await mongoose.connect(mongoUri);

    console.log('[Reset-Demo] Clearing Orders, Conversations, and Carts...');
    ...
    console.log(`[Reset-Demo] Deleted ${ordersDeleted.deletedCount} orders, ${conversationsDeleted.deletedCount} conversations, ${cartsDeleted.deletedCount} carts`);

    console.log('[Reset-Demo] Clearing and reseeding Products...');
    await Product.deleteMany({});
    const insertedProducts = await Product.insertMany(sampleProducts);
    console.log(`[Reset-Demo] Successfully reseeded ${insertedProducts.length} products`);

    console.log('[Reset-Demo] Clearing and reseeding Coupons...');
    await Coupon.deleteMany({});
    const insertedCoupons = await Coupon.insertMany(sampleCoupons);
    console.log(`[Reset-Demo] Successfully reseeded ${insertedCoupons.length} coupons`);
    ...
    console.log(`  - ${insertedCoupons.length} coupons ready`);
    console.log('\nThe system is now ready for a fresh demo run.');

    process.exit(0);
  } catch (error) {
    console.error('[Reset-Demo Error] Failed to reset demo:', error);
    process.exit(1);
  }
};
